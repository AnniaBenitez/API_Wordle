...
function jugar(){
    ...
    if(INTENTO.length === 5){
        if(!contieneNumeros(INTENTO)){        
            ...
            errorIngreso("Try again, only letters please!!"); 
        }
    }
    //En caso de quela palabra ingresada no cumpla los requisitos
    else{
        errorIngreso("Try again, words must have 5 letters!!");      
    }
}
...
//Funcion para verificar que la cadena no contenga numeros
...
//Imprime un error personalizado, según cual sea
function errorIngreso(cadenaError){
    let contenedor = document.getElementById('guesses');    
        contenedor.innerHTML = `<h2 id = "guesses">${cadenaError}</h2>`;
        setTimeout(() => {
            contenedor.innerHTML = '<h2 id = "guesses"></h2>';
        }, 5000); 
}

//Devuelve true si un caracter es numero
function isNum(str) {
    return /^\d+$/.test(str);
}